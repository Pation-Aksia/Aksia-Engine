...
void AEvent::MandatoryUpdate(HWND& handle, bool& programShouldEnd) {
    ...
    int currxkeysym; // change name to match windowsAPI
    ...
    while(XPending(handle) > 0) {
        XNextEvent(handle, &e);
        switch(e.type) {
        case KeyPress:
            currxkeysym = XkbKeycodeToKeysym(handle, e.xkey.keycode, 0, 0);
...
void AEvent::WaitUntilConfigNotify() {
    /*int tryCount = 0;
